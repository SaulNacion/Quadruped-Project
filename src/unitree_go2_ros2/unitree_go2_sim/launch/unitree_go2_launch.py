...
def generate_launch_description():
    ...
    controller_spawner_js = TimerAction(
        period=30.0,  # Wait for Gazebo to fully initialize
        actions=[
            Node(
    ...
    controller_spawner_effort = TimerAction(
        period=40.0,  # Wait 5 seconds after joint_states_controller
        actions=[
            Node(
                package="controller_manager",
                executable="spawner",
    ...
    controller_status_check = TimerAction(
        period=50.0,  # Check status after controllers should be loaded
        actions=[
            ExecuteProcess(
